...
This study examines and summarises the relationships between humidity functions, which are important for calculations used by engineers and scientists, for example to understand air conditioning systems.
...
Contour plots were utilised to visualise the interactions between derived variables across the temperature-humidity grid.
The analysis includes validation checks for temperature and relative humidity calculations, to identify potential errors.
...
### Relative Humidity `calcRH` and `calcRH_DP`
...
Using absolute humidity generates errors at low temperatures and high relative humidities that may become significant or exponentiate if used in machine learning applications.
It is recommended to be wary when using the `calcAH` function at low temperature and high relative humidity.

### Absolute humidity `calcAH`
...
### Water vapour saturation pressure `calcPws`
