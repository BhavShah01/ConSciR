...
using the International Association for the Properties of Water and Steam (IAPWS as default), Arden Buck equation (Buck) or August-Roche-Magnus approximation (Magnus).

A discussion of each formulation is given in Wikipedia and the benefits of each equation: 
...
# Graph Saturation vapor pressure using different equations
mydata |>
  dplyr::mutate(
  ...
  ggplot() +
  geom_line(aes(Date, Pw_IAPWS), col = "darkblue", alpha = 0.5) + 
  geom_line(aes(Date, Pw_Buck), col = "blue", alpha = 0.5) + 
  geom_line(aes(Date, Pw_Magnus), col = "purple", alpha = 0.5) + 
  geom_line(aes(Date, Pw_VAISALA), col = "violet", alpha = 0.5) + 
  theme_bw()
```
