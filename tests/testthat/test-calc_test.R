...
testthat::test_that("RH partial pressure check", {
  expect_equal(RH, 100 * (calcPw(Temp, RH, method = "Buck") / calcPws(Temp, method = "Buck")), tolerance = 0.0000001)
})
testthat::test_that("RH partial pressure check", {
  expect_equal(RH, 100 * (calcPw(Temp, RH, method = "IAPWS") / calcPws(Temp, method = "IAPWS")), tolerance = 0.0000001)
})
testthat::test_that("RH partial pressure check", {
